...
class ICArticlesTableViewController: ICBaseTableViewController {
    ...
    private var _cartItems: [ICCartItem] = []
    private var _selectedItems: [ICCartItem] = []
    ...
    override func paymentDidFailWithError(_ error: ICCheckoutError) {
    ...
    private func setArticles() {
        _cartItems = [
            ICCartItem(name: "Book",  price: 10.00, quantity: 1),
            ICCartItem(name: "Shirt", price: 24.00, quantity: 1),
            ICCartItem(name: "Pens",  price: 0.25,  quantity: 4),
            ICCartItem(name: "Shoes", price: 45.75, quantity: 1),
            ICCartItem(name: "Chair", price: 72.55, quantity: 1),
        ]
    }
    ...
    private var payViaCard: ((_ alertAction: UIAlertAction) -> Void)? {
        return { (_) in
            ...
            let controller = ICPaymentViewController(cartItems: self._selectedItems, orderId: orderId, delegate: self)
